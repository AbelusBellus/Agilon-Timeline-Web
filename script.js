...
// ============================================================
// 2. СОБЫТИЯ (загружаются из JSON)
// ============================================================
let EVENTS = []; // теперь это пустой массив, который заполнится асинхронно
...
function loadEvents() {
    fetch(`events.json?t=${Date.now()}`)
        ...
        .then(data => {
            EVENTS = data;
            render(); // перерисовать после загрузки
            console.log('📅 События загружены:', EVENTS.length);
        })
        .catch(error => {
            console.error('Ошибка загрузки событий:', error);
            // Если файл не найден, показываем сообщение на странице
            dom.eventsStrip.innerHTML = '<span class="placeholder">❌ Ошибка загрузки событий</span>';
        });
}
...
// Загружаем события, потом рендерим
loadEvents().then(() => {
    // рендер уже вызывается внутри loadEvents
});
